...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* Mobile menu - rendered via portal to avoid nav clipping */}
      {createPortal(
        <div
          className={cn(
          ...
          <div className="flex flex-col space-y-8 p-8 pt-10">
            {navItems.map((item) => (
              <a
                ...
                className="text-muted-foreground hover:text-foreground text-xl transition-colors duration-200 ease-in-out"
              >
                {item.name}
              </a>
            ))}
